...
disp('Time length is:');
disp(length(time)); %% the time is going to going to be 1001 with 10 secounds
...
for k = 1:length(time)
    ...
    % Computeing the predicted mean and covariance
    ...
    %For finding out if the Kalman gain stabilizes
    KGain(k) = norm(K, 'fro'); 
...
title('Maglev X Position');
legend('Estimated X (X3)', 'Measured X (Y1)', 'Predicted X');
grid on;
...
xlabel('Seconds'); ylabel('meter'); 
title('Magnet Y Position');
legend('State Estimated Y (X2)', 'Measured Y (Y2)', 'Predicted Y');
grid on;
...
xlabel('Seconds'); ylabel('meters'); 
title('Magnet Z Position');
legend('Estimated Z (X3)', 'Measured Z (Y3)', 'Predicted Z');
grid on;
