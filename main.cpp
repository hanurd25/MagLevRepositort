...
int main() {
    ...
    if (!cap.isOpened()) {
        std::cerr << "Error: Unable to open camera" << cameraPort << std::endl;
        return -1;
    }
    ...
    while (true) {
        ...
        double alpha = 0.5;
        int beta = 0;
        frame.convertTo(contrastReducedFrame, -1, alpha, beta);

        cv::Mat blurredFrame;
        cv::GaussianBlur(contrastReducedFrame, blurredFrame, cv::Size(9, 9), 0);

        cv::Mat mask = cv::Mat::zeros(frame.size(), CV_8UC1);
        //Choosing center of the mask
        cv::Point maskCenter(frame.cols / 2 + 10, frame.rows / 2 - 25);
        int maskRadius = std::min(frame.cols, frame.rows) / 6.2;
        cv::circle(mask, maskCenter, maskRadius, cv::Scalar(255), -1);
        cv::Mat maskedFrame;
        blurredFrame.copyTo(maskedFrame, mask);

        cv::Mat edges;
        cv::Canny(maskedFrame, edges, 100, 200);
        ...
        for (size_t i = 0; i < contours.size(); i++) {
            if (contours[i].size() >= 5) {
                ...
                if (distanceFromMaskCenter <= maskRadius) {
                    if (ellipse.size.width > 20 && ellipse.size.height > 20 &&
                        ...
                        cv::putText(frame, text, cv::Point(x + 10, y - 10),
